...
def send_welcome_email(user) -> bool:
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
        ...
        <p>Hola <strong>{user.first_name or user.email}</strong>,</p>
        ...
        <div style="text-align: center; margin: 30px 0;">
            <a href="http://localhost:5173/login" style="background-color: #4F46E5; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">Ingresar al Portal</a>
        </div>
...
def send_access_notification_email(user, access_log) -> bool:
    ...
    reason_info = f"<p style='color: red;'>Motivo de rechazo: {access_log.get_denial_reason_display()}</p>" if access_log.status == 'DENIED' else ""

    text_content = f"Hola {user.first_name or user.email},\n\nSe ha registrado un intento de ingreso ({access_log.status}) a las {access_log.timestamp.strftime('%H:%M:%S del %Y-%m-%d')}.\n"

    html_content = f"""
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
        <h3 style="color: {'#10B981' if access_log.status == 'GRANTED' else '#EF4444'};">Registro de Acceso ({access_log.get_status_display()})</h3>
        <p>Hola <strong>{user.first_name or user.email}</strong>,</p>
